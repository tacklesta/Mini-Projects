...
{
    let promise = fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${long}&exclude=minutely,hourly,alerts&units=imperial&appid=${weatherAPIKey}`
  ...
  export default async function fetchData(input){
      ...
      let geoPromise = fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${urlString}.json?access_token=${mapboxAPIKey}`).then((response)=>response.json());
